...
class LLMClient:
    ...
    def invoke(self, system_prompt: str, user_prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            if 'inappropriate content' in error_msg or 'content policy' in error_msg:
                logger.warning(f"[InsightEngine] 内容审查触发，尝试使用 DeepSeek 备用模型...")
                ...
                try:
                    ...
                    if response.choices and response.choices[0].message:
                        logger.info("[InsightEngine] DeepSeek 备用模型调用成功")
                        return self.validate_response(response.choices[0].message.content)
                    return ""

                except Exception as deepseek_error:
                    logger.error(f"[InsightEngine] DeepSeek 备用模型也失败: {deepseek_error}")
                    raise
    ...
    def stream_invoke_to_string(self, system_prompt: str, user_prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            # 检测内容审查错误，使用 DeepSeek 备用模型
            if 'inappropriate content' in error_msg or 'content policy' in error_msg:
                logger.warning(f"[InsightEngine][stream] 内容审查触发，尝试使用 DeepSeek 备用模型...")
                ...
                try:
                    ...
                    if byte_chunks:
                        logger.info("[InsightEngine] DeepSeek 备用模型流式调用成功")
                        return b''.join(byte_chunks).decode('utf-8', errors='replace')
                    return ""
